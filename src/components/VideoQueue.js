    ...
    makeList(){
        ...
        this.state.queue.forEach( (vid,idx) => {
            ...
            if(this.state.current == idx){
                item = (<ListGroup.Item as={"li"} style={{ backgroundColor: '#18474E', color:'#8EE3F1' }}   onClick={()=>{
                ...
                item = (<ListGroup.Item as={"li"} style={{ backgroundColor: '#0f2124', color: '#8EE3F1' }}  onClick={()=>{
                    if(!this.state.is_join) {
                        this.setState({"current": idx});
                        this.state.changeVideo(vid);
                    }
